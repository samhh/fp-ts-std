...
```ts
export declare const elemFlipped: <A>(eq: Eq<A>) => (xs: readonly A[]) => Predicate<A>
```
...
```ts
export declare const length: (xs: ReadonlyArray<unknown>) => number
```

```hs
length :: ReadonlyArray unknown -> number
```
...
```ts
export declare const moveTo: (to: number) => (from: number) => <A>(xs: readonly A[]) => Option<readonly A[]>
```
